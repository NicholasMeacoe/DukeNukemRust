...
impl Plugin for SaveLoadPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<SaveManager>()
            ...
            .add_systems(Update, (handle_save_load_hotkeys, handle_save_events, handle_load_events, auto_tag_saveable, hydrate_enemies, hydrate_items, hydrate_switches_and_props, hydrate_projectiles, hydrate_effectors))
            .add_systems(Update, apply_pending_save.after(crate::game_flow::level_loader::handle_load_level_events));
    }
}
...
pub fn handle_save_events(
    ...
    q_group1: (
        Query<(&Transform, &crate::combat::types::EnemyActor, &crate::scripting::ConActor), With<Saveable>>,
        ...
        Query<(&Transform, &crate::interactivity::types::SectorEffectorComponent), With<Saveable>>,
    ...
    q_group2: (
        ...
        Query<(&Transform, &crate::interactivity::props_extended::DancerProp), With<Saveable>>,
        ...
        Query<(&Transform, &crate::interactivity::props_extended::SecurityCameraMonitor), With<Saveable>>,
    ...
    for event in events.read() {
        ...
        for (t, e, c) in &q_group1.0 { snapshot.enemies.push(((*t).into(), e.clone(), c.clone())); }
        for (t, i) in &q_group1.1 { snapshot.items.push(((*t).into(), i.clone())); }
        for (t, p) in &q_group1.2 { snapshot.projectiles.push(((*t).into(), p.clone())); }
        for (t, e) in &q_group1.3 { snapshot.sector_effectors.push(((*t).into(), e.clone())); }
        for (t, s) in &q_group1.4 { snapshot.switches.push(((*t).into(), s.clone())); }
        for (t, c) in &q_group1.5 { snapshot.crack_walls.push(((*t).into(), c.clone())); }
        for (t, g) in &q_group1.6 { snapshot.glass_panes.push(((*t).into(), g.clone())); }
        for (t, b) in &q_group1.7 { snapshot.barrels.push(((*t).into(), b.clone())); }
        
        for (t, f) in &q_group2.0 { snapshot.fountains.push(((*t).into(), f.clone())); }
        for (t, tp) in &q_group2.1 { snapshot.toilets.push(((*t).into(), tp.clone())); }
        for (t, d) in &q_group2.2 { snapshot.dancers.push(((*t).into(), d.clone())); }
        for (t, v) in &q_group2.3 { snapshot.viewscreens.push(((*t).into(), v.clone())); }
        for (t, c) in &q_group2.4 { snapshot.cameras.push(((*t).into(), c.clone())); }
        for (t, m) in &q_group2.5 { snapshot.camera_monitors.push(((*t).into(), m.clone())); }
        for (t, n) in &q_group2.6 { snapshot.nuke_switches.push(((*t).into(), n.clone())); }
        for (t, m) in &q_group2.7 { snapshot.master_switches.push(((*t).into(), m.clone())); }
...
pub fn apply_pending_save(
    ...
    let Some(pending_res) = pending else { return; };
    ...
    // Restore Dynamics
    for (st, e, c) in &snapshot.enemies { commands.spawn((Into::<Transform>::into(st.clone()), e.clone(), c.clone(), Saveable, RequiresHydration, crate::game_flow::state::LevelEntity)); }
    for (st, i) in &snapshot.items { commands.spawn((Into::<Transform>::into(st.clone()), i.clone(), Saveable, RequiresHydration, crate::game_flow::state::LevelEntity)); }
    for (st, p) in &snapshot.projectiles { commands.spawn((Into::<Transform>::into(st.clone()), p.clone(), Saveable, RequiresHydration, crate::game_flow::state::LevelEntity)); }
    for (st, e) in &snapshot.sector_effectors { commands.spawn((Into::<Transform>::into(st.clone()), e.clone(), Saveable, RequiresHydration, crate::game_flow::state::LevelEntity)); }
    for (st, s) in &snapshot.switches { commands.spawn((Into::<Transform>::into(st.clone()), s.clone(), Saveable, RequiresHydration, crate::game_flow::state::LevelEntity)); }
    for (st, c) in &snapshot.crack_walls { commands.spawn((Into::<Transform>::into(st.clone()), c.clone(), Saveable, RequiresHydration, crate::game_flow::state::LevelEntity)); }
    for (st, g) in &snapshot.glass_panes { commands.spawn((Into::<Transform>::into(st.clone()), g.clone(), Saveable, RequiresHydration, crate::game_flow::state::LevelEntity)); }
    for (st, b) in &snapshot.barrels { commands.spawn((Into::<Transform>::into(st.clone()), b.clone(), Saveable, RequiresHydration, crate::game_flow::state::LevelEntity)); }
    for (st, f) in &snapshot.fountains { commands.spawn((Into::<Transform>::into(st.clone()), f.clone(), Saveable, RequiresHydration, crate::game_flow::state::LevelEntity)); }
    for (st, tp) in &snapshot.toilets { commands.spawn((Into::<Transform>::into(st.clone()), tp.clone(), Saveable, RequiresHydration, crate::game_flow::state::LevelEntity)); }
    for (st, d) in &snapshot.dancers { commands.spawn((Into::<Transform>::into(st.clone()), d.clone(), Saveable, RequiresHydration, crate::game_flow::state::LevelEntity)); }
    for (st, v) in &snapshot.viewscreens { commands.spawn((Into::<Transform>::into(st.clone()), v.clone(), Saveable, RequiresHydration, crate::game_flow::state::LevelEntity)); }
    for (st, c) in &snapshot.cameras { commands.spawn((Into::<Transform>::into(st.clone()), c.clone(), Saveable, RequiresHydration, crate::game_flow::state::LevelEntity)); }
    for (st, m) in &snapshot.camera_monitors { commands.spawn((Into::<Transform>::into(st.clone()), m.clone(), Saveable, RequiresHydration, crate::game_flow::state::LevelEntity)); }
    for (st, n) in &snapshot.nuke_switches { commands.spawn((Into::<Transform>::into(st.clone()), n.clone(), Saveable, RequiresHydration, crate::game_flow::state::LevelEntity)); }
    for (st, m) in &snapshot.master_switches { commands.spawn((Into::<Transform>::into(st.clone()), m.clone(), Saveable, RequiresHydration, crate::game_flow::state::LevelEntity)); }
...
pub fn auto_tag_saveable(
    mut commands: Commands,
    q1: Query<Entity, (Or<(With<crate::combat::types::EnemyActor>, With<crate::interactivity::types::ItemPickup>, With<crate::combat::types::Projectile>, With<crate::interactivity::types::SectorEffectorComponent>, With<crate::interactivity::types::InteractiveSwitch>, With<crate::interactivity::types::CrackWall>, With<crate::interactivity::types::BreakableGlass>, With<crate::interactivity::types::ExplodingBarrel>)>, Without<Saveable>)>,
    q2: Query<Entity, (Or<(With<crate::interactivity::types::WaterFountain>, With<crate::interactivity::types::ToiletProp>, With<crate::interactivity::props_extended::DancerProp>, With<crate::interactivity::types::ViewscreenProp>, With<crate::interactivity::types::SecurityCamera>, With<crate::interactivity::props_extended::SecurityCameraMonitor>, With<crate::interactivity::types::NukeExitSwitch>, With<crate::interactivity::types::MasterSwitch>)>, Without<Saveable>)>,
) {
    for e in &q1 { commands.entity(e).insert(Saveable); }
    for e in &q2 { commands.entity(e).insert(Saveable); }
}

pub fn hydrate_enemies(
    mut commands: Commands,
    query: Query<(Entity, Option<&crate::scripting::ConActor>), (With<RequiresHydration>, With<crate::combat::types::EnemyActor>)>,
    assets: Option<Res<crate::GameAssets>>,
    mut meshes: ResMut<Assets<Mesh>>,
    mut materials: ResMut<Assets<StandardMaterial>>,
) {
    let Some(game_assets) = assets else { return; };
    let quad_mesh = meshes.add(Rectangle::new(1.0, 1.0));
    for (e, con) in &query {
        let picnum = con.map_or(2000, |c| c.picnum);
        let mat = game_assets.tile_textures.get(&picnum).cloned().map(|t| materials.add(StandardMaterial { base_color_texture: Some(t), alpha_mode: AlphaMode::Mask(0.5), unlit: true, ..default() })).unwrap_or(game_assets.default_material.clone());
        commands.entity(e).insert((PbrBundle { mesh: quad_mesh.clone(), material: mat, ..default() }, crate::SpriteBillboard, RigidBody::Fixed, Collider::cuboid(0.5, 0.5, 0.1), crate::Destructible { health: 100, _picnum: picnum }));
...
pub fn hydrate_items(
    ...
    for (e, item) in &query {
        ...
        let mat = game_assets.tile_textures.get(&picnum).cloned().map(|t| materials.add(StandardMaterial { base_color_texture: Some(t), alpha_mode: AlphaMode::Mask(0.5), unlit: true, ..default() })).unwrap_or(game_assets.default_material.clone());
        commands.entity(e).insert((PbrBundle { mesh: quad_mesh.clone(), material: mat, ..default() }, crate::SpriteBillboard, RigidBody::Fixed, Collider::cuboid(0.5, 0.5, 0.1), crate::Destructible { health: 10, _picnum: picnum }));
        commands.entity(e).remove::<RequiresHydration>();
    }
}
...
pub fn hydrate_switches_and_props(
    ...
    let Some(game_assets) = assets else { return; };
    let quad_mesh = meshes.add(Rectangle::new(1.0, 1.0));
    for (e, sw, crack, glass, barrel, fountain, toilet, dancer, viewscreen, camera, monitor, nuke, master) in &query {
        let mut picnum = -1;
        if let Some(s) = sw { picnum = if s.is_on { s.on_tile } else { s.off_tile }; }
        else if crack.is_some() { picnum = 546; }
        else if glass.is_some() { picnum = 502; }
        else if barrel.is_some() { picnum = 122; }
        else if fountain.is_some() { picnum = 564; }
        else if toilet.is_some() { picnum = 1210; }
        else if dancer.is_some() { picnum = 1324; }
        else if viewscreen.is_some() { picnum = 660; }
        else if camera.is_some() { picnum = 650; }
        else if monitor.is_some() { picnum = 660; }
        else if nuke.is_some() { picnum = 142; }
        else if master.is_some() { picnum = 130; }
...
pub fn hydrate_effectors(
    mut commands: Commands,
    query: Query<Entity, (With<RequiresHydration>, With<crate::interactivity::types::SectorEffectorComponent>)>,
) {
    for e in &query {
        commands.entity(e).remove::<RequiresHydration>();
    }
...
mod tests {
    ...
    fn test_save_load_game_snapshot_roundtrip() {
        ...
        snapshot.player = Some((SavedTransform { translation: [-15.5, 4.2, 8.0], rotation: [0.0, 0.0, 0.0, 1.0], scale: [1.0, 1.0, 1.0] }, player));

        let bytes = bincode::serialize(&snapshot).expect("Failed to serialize");
        let loaded: SaveGameSnapshot = bincode::deserialize(&bytes).expect("Failed to deserialize snapshot");
